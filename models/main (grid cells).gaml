...
global {
	bool mqtt_connect <- false; //conexion MQTT

	// Benchmark execution time
	bool benchmark <- false; //prueba de rendimiento
	float time_absorb_pollutants; //tiempo de absorcion de contaminantes
	float time_diffuse_pollutants;//tiempo de difusión de contaminantes
	float time_create_congestions; //tiempo de creación de congestionantes

	float step <- 16#s; //paso del tiempo en segundos
	date starting_date <- date(starting_date_string,"HH mm ss"); //fecha de inicio en segundos
...
//	shape_file buildings_shape_file <- shape_file("../includes/_old_dataset/map3D/HKA_maquette - buildings.shp");
	...
	list<road> open_roads; //lista de calles abiertas
	...
	init {
		...
		create progress_bar    with: [x::3100, y::900, width::350, height::100, max_val::500, title::"Hibridos", left_label::"0", right_label::"Max"];
		...
		create param_indicator with: [x::3100, y::450, size::22, name::"Polucion", value::"", with_RT::false];
		create param_indicator with: [x::3100, y::600, size::22, name::"AQI_LEVELS", value::"", with_RT::false];
	}
	
	action update_vehicle_population(string type, int delta) {//actualiza la poblacion de vehiculos (motos o autos)
	...
	reflex update_vehicle_population_according_to_daytime when:day_time_traffic {//se activa cuando es de día
		float t_rate <- general_traffic_daytime(); //tasa de tráfico durante el día
		n_cars <- int(max_number_of_cars * t_rate); //número de autos
		n_motorbikes <- int(max_number_of_motorbikes * t_rate); //número de motocicletas en función a la tasa
		n_hibridos <- int(max_number_of_hibridos * t_rate); //numero de hibridos 
	}
	
	reflex update_car_population when: n_cars != n_cars_prev {//se activa cuando el número de autos ha cambiado
		int delta_cars <- n_cars - n_cars_prev;
		do update_vehicle_population("car", delta_cars);//actualiza la poblacion de vehiculos car
		ask first(progress_bar where (each.title = "Cars")) {//actualiza la barra de progreso
			do update(float(n_cars));
		}
		n_cars_prev <- n_cars;//actualiza el valor de carros previstos
	}
	
	//poblacion de hibridos	
	reflex update_hibridos_population when: n_hibridos != n_hibridos_prev {//se activa cuando el número de hibridos ha cambiado
		int delta_hibridos <- n_hibridos - n_hibridos_prev;
		do update_vehicle_population("hibrido", delta_hibridos);
		ask first(progress_bar where (each.title = "Hibridos")) {
			do update(float(n_hibridos));
		}
		n_hibridos_prev <- n_hibridos;
	}
	
	reflex update_motorbike_population when: n_motorbikes != n_motorbikes_prev {//lo mismo que el de arriba pero con motos
	...
	reflex update_road_scenario when: road_scenario != road_scenario_prev {//se activa cuando el escenario ha cambiado
		//se actualiza la losta de calles abiertas
		string param_val;
		...
		// Recrear la red de carreteras,recalcula las rutas de vehiculos y se actualiza la visulizacion
	...
	reflex update_display_mode when: display_mode_prev != display_mode {//se activa cuando el modo de visualizacion cambia
		string param_val;
		switch (display_mode) {
			match 0 {
				param_val <- "Traffic";
	...
	reflex update_time { //formatea lahora
	...
	reflex update_building_aqi {//se activa para cada edificio 
		ask building parallel: true {
			aqi <- pollutant_cell(p_cell).aqi; //p_cell, celda de contaminante más cercana al edificio
		}
	}
	
	reflex create_congestions {//calcula la cantidad de vehiculos en cada carretea y actualiza el coeficiente de velocidad para reflejar congestiones
		...
		ask open_roads {
			...
			int n_hibridos_on_road <- vehicles_on_road count (each.type = "hibridos"); //agregar hibridos
			do update_speed_coeff(n_cars_on_road, n_motorbikes_on_road, n_hibridos_on_road);
		}
	...
	reflex produce_pollutant {//absorbe los contaminantes en las celdas activas
		float start <- machine_time;
	...
	reflex calculate_aqi when: every(refreshing_rate_plot) { //every(1 #minute) {
		 ...
		 ask first(param_indicator where (each.name = "Polucion")) {
			do update(string(aqi with_precision 2));
		}
		
		ask first(param_indicator where (each.name = "AQI_LEVELS")) {
			do update(world.get_pollution_state(aqi));
		}
	 ...
	 */
	rgb day_time_color <- #black; //color de fondo inicia en negro
...
experiment exp autorun: true {
	...
	parameter "Numero de hibridos" var: n_hibridos <- 0 min: 0 max: 500;
